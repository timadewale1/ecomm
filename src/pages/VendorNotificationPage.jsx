  ...
  useEffect(() => {
    const fetchNotifications = async (vendorId) => {
      try {
        ...
        const q = query(notificationsRefDB, where("vendorId", "==", vendorId));
    ...
    const unsubscribe = onAuthStateChanged(auth, (vendor) => {
      if (vendor) {
        setCurrentvendor(vendor);
        fetchNotifications(vendor.uid);
      } else {
        navigate("/login");
      }
    });
