...
const notifyOrderStatusChange = async (
  ...
  try {
    ...
    console.log("Rider Info before notification creation:", riderInfo);
    ...
    console.log("Rider Info before sending to Cloud Function:", riderInfo);

    // Call the API to send an email notification
    console.log("Sending email notification via Cloud Function...");
    await axios.post(
      "https://us-central1-ecommerce-ba520.cloudfunctions.net/sendNotification",
      {
        userId,
        orderId,
        newStatus,
    ...
    console.error(
      "Error notifying user about order status change:",
      error.message
    );
  }
};
