  ...
  useEffect(() => {
    const checkCartProducts = async () => {
      ...
      for (const productId of productIds) {
        const productDoc = await getDoc(doc(db, `vendors/${cart[productId].vendorId}/products`, productId));
        if (!productDoc.exists()) {
          delete updatedCart[productId];
          dispatch(removeFromCart(productId));
          toast.info(`Product ${cart[productId].name} has been removed as it is no longer available.`);
        }
      }
