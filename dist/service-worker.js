...
self.addEventListener("fetch", (event) => {
  // (kept) simple bot check — harmless if header not present
  const userAgent = event.request.headers.get("User-Agent") || "";
  const isBot = /bot|crawler|spider|crawling/i.test(userAgent);
  if (isBot) return;

  // ✅ Don't touch non-GET requests (POST/OPTIONS to Cloud Functions, preflights)
    ...
    fetch(event.request)
      ...
      .catch(() => {
        ...
        return caches.match(event.request).then((res) => {
          ...
          // Fallback to offline page for navigations/HTML
          const isHTML =
