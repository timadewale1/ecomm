...
const RoleBasedAccess = ({ allowedRoles, children }) => {
  ...
  const { setHideBottomBar } = useContext(AccessContext); // Get setHideBottomBar from context
  ...
  if (currentUser && !allowedRoles.includes(currentUserData?.role)) {
    return (
      <div className="flex justify-center flex-col translate-y-44 items-center ">
        ...
        <button className="bg-customOrange text-xs font-medium text-white py-2 px-2 rounded-lg ">
          Back to Dashboard
        </button>
      </div>
    );
